...
class GPUModelRunner(
    ...
    def __init__(
        ...
        # Compatibility with the older ngram-gpu async path. These stay nullable
        # in the merged implementation and are only populated when needed.
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> None:
        ...
        for i, req_id in enumerate(req_data.req_ids):
            ...
            # Update spec token state for the request.
            self.input_batch.update_req_spec_token_ids(req_state, scheduled_spec_tokens)

            # Restore scheduler-side draft count after ngram trimming so
            # rejection correction keeps using the optimistic scheduled length.
            ...
            if self.use_async_scheduling and self.use_gpu_input_correction:
        ...
        # Build curr_to_prev_idx mapping for GPU input correction after
        # condense/reorder so it reflects the final batch order.
    ...
    def execute_model(
        ...
        # mutating the engine-core side metadata in place.
        if (
            self.speculative_config is not None
            and self.speculative_config.use_ngram_gpu()
        ):
    ...
    def sample_tokens(
        ...
        if spec_config is not None:
            ...
            if use_gpu_toks:
                # EAGLE/DraftModel/ExtractHiddenStates speculative decoding can use
                # the GPU sampled tokens as inputs, and does not need to wait for
    ...
    def propose_draft_token_ids(
        ...
        draft_batch_size: int | None = None
        if spec_config.method == "ngram":
            ...
            draft_batch_size = next_token_ids.shape[0]

            draft_token_ids, num_valid_draft_tokens = self.drafter.propose(
                self.num_tokens_no_spec_gpu[:draft_batch_size],
                self.token_ids_gpu_tensor[:draft_batch_size],
                valid_sampled_token_ids_gpu,
                valid_sampled_tokens_count,
            )
        ...
        if self.use_gpu_input_correction and draft_batch_size is not None:
            with record_function_or_nullcontext(
                "gpu_model_runner: cache_prev_num_draft_len_gpu"
            ):
                self._cache_prev_num_draft_len_gpu(scheduler_output, draft_batch_size)
